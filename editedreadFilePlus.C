...
void editedreadFilePlus() {
  ...
  //c3->Divide(3,4);
  TCanvas *c4=new TCanvas("c4","",800,800);//We only care about this Canvas
  c4->Divide(3,4);// will contain multiple graphs
  ...
  for (Int_t j = 0; j<9; j++) {//pt loop start   Int_t j = 0; j<9; j++ #####################################################################################################
    ...
    h1d = (TH1D*) h3->ProjectionX(Form("p_{T} : %3.1f-%3.1f GeV/c",xa[j], xa[j+1]),
                                                h3->GetYaxis()->FindBin(xa[j]+0.001),
                                                h3->GetYaxis()->FindBin(xa[j+1]-0.001), h3->GetZaxis()->FindBin(0+0.001), h3->GetZaxis()->FindBin(10-0.001), "e");//multiplicity 0-10
  ...
  mass->Write();
  width->Write();
  rawYield->Write();//We want this graph also
  hchi2->Write();
