...
void readFilePlus() {
  ...
  //TList *l1 = (TList*) f1->Get("RsnOut_SigmaPlusMinus;1"); //wasn't commented before
  // for MB minimum bias, multiplicity 0-90
  TList *l1 = (TList*) f1->Get("RsnOut_SigmaPMkINT7;1"); //was commented before

 // 
  TH1F *hAEventsVsMulti= (TH1F *) l1->FindObject("hAEventsVsMulti");
  ...
  TH2F *hVzVsCent1 = (TH2F*) l1->FindObject("hVzVsCent");
  
  TH1D *vZ0_90_1= (TH1D *) hVzVsCent1->ProjectionY(Form("cent : %3.1f-%3.1f GeV/c",hVzVsCent1->GetXaxis()->GetBinCenter(hVzVsCent1->GetXaxis()->FindBin(0+0.001)),
							hVzVsCent1->GetXaxis()->GetBinCenter(hVzVsCent1->GetXaxis()->FindBin(90-0.001))), hVzVsCent1->GetXaxis()->FindBin(0+0.001), hVzVsCent1->GetXaxis()->FindBin(90-0.001), "e");
  
  TH1D *vZ0_90= new TH1D(*vZ0_90_1);
  ...
  TF1 *fau1=new TF1("fau1", BreitWignerCombFit, 1.2, 2., 7);
  ...
  for (Int_t j = 0; j<9; j++) {//pt loop start Int_t j = 0; j<9; j++ #####################################################################################################
    ...
    TH1D *h1d = (TH1D*) h3->ProjectionX(Form("p_{T} : %3.1f-%3.1f GeV/c",xa[j], xa[j+1]),
                                                      h3->GetYaxis()->FindBin(xa[j]+0.001),
                                                      h3->GetYaxis()->FindBin(xa[j+1]-0.001), h3->GetZaxis()->FindBin(0+0.001), h3->GetZaxis()->FindBin(90-0.001), "e");
    ...
    h1d = (TH1D*) h3->ProjectionX(Form("p_{T} : %3.1f-%3.1f GeV/c",xa[j], xa[j+1]),
			       				h3->GetYaxis()->FindBin(xa[j]+0.001),
			       				h3->GetYaxis()->FindBin(xa[j+1]-0.001), h3->GetZaxis()->FindBin(0+0.001), h3->GetZaxis()->FindBin(90-0.001), "e");//multiplicity 0-10
    ...
    TF1 *fu=new TF1("fu", BreitWignerCombFit, 1.2, 2., 7);//u can change the limits here
    ...
    c4->Print("sigmaPlusInvMasses0_90.pdf");
    
    TFile* outputFile = TFile::Open("sigmaPlusProperties0_90.root", "recreate");
